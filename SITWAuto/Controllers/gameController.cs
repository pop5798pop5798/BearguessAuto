...
namespace SIT.Controllers
{
    ...
    public class gameController : Controller
    {
        ...
        public async System.Threading.Tasks.Task<GamePostViewModel> AutoEdit(int id)
        {
            GamePosts gamepost = new GamePostsRepository().get(id);
            gameDto game = null;
            string UserID = new AccountRepository().getUserEmail("pop5798pop5798@gmail.com").Id;
            game = await new GamesRepository().GetGameDetail(gamepost.GameSn);
            //if (game.userId != UserID)
            //    return View("Details", game);
        ...
        [HttpPost]
        public async System.Threading.Tasks.Task<ActionResult> AutoEdit(GamePostViewModel gpvm)
        {
            //try
            //{

            gameDto game = gpvm.game;
            game.userId = new AccountRepository().getUserEmail("pop5798pop5798@gmail.com").Id;
            game.comSn = 1;
            game = await new GamesRepository().Edit(game.sn, game);
            ...
            gpvm.gamepost.TeamASn = gpvm.gamepost.TeamASn;
            gpvm.gamepost.TeamBSn = gpvm.gamepost.TeamBSn;
            gpvm.gamepost.sdate = gpvm.game.sdate;
            gpvm.gamepost.edate = gpvm.game.edate;
            gpvm.gamepost.PlayGameSn = gpvm.gamepost.PlayGameSn;
            new GamePostsRepository().update(gpvm.gamepost);
