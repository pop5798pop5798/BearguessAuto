...
namespace SIT.Controllers
{
    ...
    public class gameController : Controller
    {
        ...
        // GET: Home
        [AllowAnonymous]
        public async System.Threading.Tasks.Task<ActionResult> GuessGame()
        {
            ...
            var gpvmListDy = gpvmList.Where(x=>x.game.betModel == 10);
        ...
        public async System.Threading.Tasks.Task<ActionResult> Index()
        {
            ...
            foreach (var gp in gpList)
            {
                    ...
                    gpvm.gamepost = gp;
                    gpvm.game = gameList.Where(p => p.sn == gp.GameSn).FirstOrDefault();
                    if (gpvm.game == null || gpvm.game.betModel == 10)
                       continue;
                    if ((gp.sdate <= DateTime.Now && gp.edate >= dt) || gpvm.game.betModel == 7 && gpvm.game.betModel != 10)
                    {
                        ...
                        else
                        {
                            ...
                            string movieclass_youtube = "";
                            string movieclass_huya = "";
                            string movieclass_fish = "";
                            if (gpvm.vedio.vediourl.Length >= 22)
                            {
                                movieclass_twitch = gpvm.vedio.vediourl.Substring(0, 22);
                                movieclass_fish = gpvm.vedio.vediourl.Substring(0, 22);
                            }
                            if (gpvm.vedio.vediourl.Length >= 24)
                            {
                                movieclass_youtube = gpvm.vedio.vediourl.Substring(0, 24);
                        ...
                        else if (movieclass_fish == "https://www.douyu.com/")
                        {
                            gpvm.vedio_url = Regex.Replace(gpvm.vedio.vediourl, movieclass_fish + "room/share/", String.Empty);
                            gpvm.vedio_url = "https://tc-tct.douyucdn2.cn/dyliveflv1a/" + gpvm.vedio_url + "rimbadota_4000.flv?wsAuth=64fc9a7d0cdff8bd01c79a433cf79bf3&token=web-h5-0-19002-3db56ab40936dfa5735e401bf68654849260bcf9e307e88e&logo=0&expire=0&did=310dc1b562727db10f08d28900041501&ver=Douyu_219103005&pt=2&st=0&origin=all&mix=0&isp=";
                            gpvm.vedeotype = 4;
                        }
                        else
                        {
        ...
        public async System.Threading.Tasks.Task<ActionResult> Details(int id)
        {
            ...
            if (Session[strUserSessionID + "LastAccessTime_" + id] == null)
            {
                /*if (ViewData["TotalUsers_"+id] == null)
                {
                    //所有線上用戶數量為0
                    ViewData["TotalUsers_" + id] = 0;
                }
                ArrayList arrltOnlineUserC = (ArrayList)ViewData["OnlineUser_"+id];
                //設定累計數值值為 0*/

                arrltTemp.Add(strUserSessionID);
                Session[strUserSessionID + "LastAccessTime_" + id] = arrltTemp;
                var gp = new GamePostsRepository().get(id);
                gp.LiveCount = (gp.LiveCount == null) ? 0 : gp.LiveCount;
                gp.LiveCount = gp.LiveCount + 1;
        ...
        {           
            ...
            var setting = await new SttingRepository().GetTopicSettingList();
            setting.topicsettingList = setting.topicsettingList.ToList();
            GamePostViewModel gpvm = new GamePostViewModel { settingList = setting, game = game, vedio = new VedioRecord(),TeamList = new List<Teams>()};
        ...
        [Authorize(Roles = "Admin")]
        public async Task<ActionResult> _topicCreate(gameDto model, int? index,int? walk, int? choice,SettingListDto tList)
        {
            index = index ?? 0;
            ViewBag.Index = index;
            ViewBag.walk = walk;
        ...
        public async System.Threading.Tasks.Task<ActionResult> Create(GamePostViewModel gpvm)
        {
            try
            {
                ...
                var leaguejudge = (gpvm.gamepost.PlayGameSn == 1) ? "LOL未分類" : (gpvm.gamepost.PlayGameSn == 2) ? "CS:GO未分類" : (gpvm.gamepost.PlayGameSn == 4) ? "DOTA2未分類":"";
                var teamA = new Teams();
                var teamB = new Teams();
                if (gpvm.gamepost.TeamASn != null)
                    teamA = new TeamsRepository().Get((int)gpvm.gamepost.TeamASn);
                if (gpvm.gamepost.TeamBSn != null)
                    teamB = new TeamsRepository().Get((int)gpvm.gamepost.TeamBSn);
                ...
                if (gpvm.TeamList[0].name != null)
                {
                    ...
                    if (gpvm.TeamList[0].shortName == null)
                    {
                        gpvm.TeamList[0].shortName = gpvm.TeamList[0].name;
                    }
                    int teamsn = new TeamsRepository().AutoCreate(gpvm.TeamList[0]);
                    gpvm.gamepost.TeamASn = teamsn;

                }
                else if (teamA.imageURL != gpvm.TeamList[0].imageURL)
                {

                    teamA.imageURL = gpvm.TeamList[0].imageURL;
                    new TeamsRepository().Update(teamA);
                }
                ...
                if (gpvm.TeamList[1].name != null)
                {
                    ...
                    if(gpvm.TeamList[1].shortName == null)
                    {
                        gpvm.TeamList[1].shortName = gpvm.TeamList[1].name;
                    }
                    int teamsn = new TeamsRepository().AutoCreate(gpvm.TeamList[1]);
                    gpvm.gamepost.TeamBSn = teamsn;
                }else if(teamB.imageURL != gpvm.TeamList[1].imageURL)
                {
                    teamB.imageURL = gpvm.TeamList[1].imageURL;
                    new TeamsRepository().Update(teamB);
                }
                if(gpvm.walkm == 1)
                {
        ...
        public async System.Threading.Tasks.Task<List<topicDto>> nabobcreate(GamePostViewModel gpvm) {            
            ...
            //增加及分辨選項
            int ti = 0;
            foreach (var topic in gameb.topicsettingList.Where(x=>x.gametype == 1))
            {
                ...
                else
                {
                    ...
                    foreach (var choiceall in topic.choicsettingList)
                    {
                        ...
                        {
                            if(i == 0)
                                choice.choiceStr = gpvm.c_choice1;
                            else
                                choice.choiceStr = gpvm.c_choice2;
                        }                           
                        else
                        {
